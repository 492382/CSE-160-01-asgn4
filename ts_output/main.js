...
async function main() {
    ...
    let animation_loop = (timestamp_milis) => {
        ...
        scene.set_matrix(scene.u_CameraMatrix, matrix_list_multiply([rot_mat, make_translation_matrix(-camera_pos[0], -camera_pos[1], -camera_pos[2])]));
        if (auto_move_light) {
...
function addUiCallbacks() {
    ...
    document.getElementById("lighting_checkbox").addEventListener("input", function (event) {
        scene.set_do_lighting(event.target.checked);
    });
    document.getElementById("visualize_normals_checkbox").addEventListener("input", function (event) {
        visualize_normals = event.target.checked;
    });
    let move_lighting_checkbox = document.getElementById("move_light_checkbox");
    move_lighting_checkbox.addEventListener("input", function (event) {
        auto_move_light = event.target.checked;
    });
...
function set_sliders_values() {
    ...
    document.getElementById("light_Y_slider").value = light_pos[1].toString();
    document.getElementById("light_Z_slider").value = light_pos[2].toString();
}
